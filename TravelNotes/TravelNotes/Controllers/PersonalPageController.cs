...
namespace TravelNotes.Controllers
{
    public class PersonalPageController : Controller
    {
        ...
        public IActionResult PersonalPage(string? search, int? userId)
        {
            ...
            var articlesQuery = _context.article
    .Where(a => a.ArticleState == "發佈");
            if (users != null)
            {
                articlesQuery = articlesQuery.Where(a => a.UserId == users.UserId);
            }
            else
            {
                // 如果使用者為空，可以直接返回一個空的列表
                articlesQuery = articlesQuery.Where(a => false);
            }
